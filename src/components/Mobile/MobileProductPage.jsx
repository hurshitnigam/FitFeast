...
const MobileProductPage = () => {
    ...
    const productData = [
        {
            ...
            imgWidth: '', imgHeight: '400px',
            imagePosition: 'bottom-8',
        ...
        },
        {
            ...
            imgWidth: '', imgHeight: '400px',
            imagePosition: 'bottom-10 left-8',
    ...
    return (
        <section ref={sectionRef} className="relative md:h-[300vh] bg-[#FAEADE]">
            ...
            <div className="md:sticky md:top-0 md:h-screen overflow-hidden">
                ...
                <motion.div
                    ref={horizontalRef}
                    style={{ x: scrollX }}
                    className="flex md:flex-nowrap flex-wrap md:gap-10 gap-8 md:min-h-screen w-full md:w-max md:px-10 px-4 py-10"
                >

                    <div className="font-bold flex flex-col items-start justify-center md:min-h-screen
                    md:p-10 p-4 md:ml-50 text-5xl md:text-9xl font-[Antonio]">
                        <div className="flex flex-col items-center gap-2 -tracking-normal text-[#523122] font-bold">
                            ...
                            <h1 className="ml-18">Delicious Flavours</h1>
                        </div>
                    </div>

                    {productData.map((product) => (
                        <Link to={`/m-product${product.id}`} key={product.id} className="w-full md:w-auto">
                            <motion.div
                                className={`relative w-full md:w-[650px] h-[400px] md:h-[500px] ${product.color} rounded-xl shadow-xl`}
                            ...
                            >
                                ...
                                />
                                <h2 className={`uppercase rounded-3xl absolute ${product.textPosition} ${product.textBgColor} ${product.textColor} ${product.textPadding} text-2xl md:text-4xl font-medium`}>
                                    {product.name}
                                </h2>
                            </motion.div>
                        </Link>
